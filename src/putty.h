...
#define IDM_CONNECT                     40018
#define IDM_PRISCR                      41019
...
typedef enum _Mouse_Button {
	MBT_NOTHING, MBT_LEFT, MBT_MIDDLE, MBT_RIGHT, /* `raw' button designations */
	MBT_SELECT, MBT_EXTEND, MBT_PASTE, MBT_COPY, /* `cooked' button designations */
...
enum {
	 ...
	 */
	KEX_WARN, KEX_DHGROUP1, KEX_DHGROUP14, KEX_DHGEX, KEX_MAX
};
...
enum {
	...
	CIPHER_3DES, CIPHER_BLOWFISH, CIPHER_AES, /* (SSH-2 only) */
	CIPHER_DES, CIPHER_ARCFOUR, CIPHER_MAX /* no. ciphers (inc warn) */
};
...
enum {
	...
	FUNKY_TILDE, FUNKY_LINUX, FUNKY_XTERM, FUNKY_VT400, FUNKY_VT100P, FUNKY_SCO
};
...
struct backend_tag {
	...
	int (*send_bin)(void *handle, byte *buf, int len);
...
#define KEYCODEFXX_MAX_SIZE 16
#define KEYCODEFXX_MAX_NUM 13
#define KEY_LEN 10
#define NAME_LEN 128
...
struct config_tag {
	...
	int useselfsety,bandy;
	int useselfsetz,bandz;
	int useselfsetx,bandx;
	int useselfsetu,bandu;
	int useselfsetw,bandw;
	int useselfsetv,bandv;
	int debug;
//	int returnpaper;
	char updateip[NAME_LEN];
	int updateport;
	char sysuser[NAME_LEN];
	char syspass[NAME_LEN];
	int currentuser;
	...
	int admin;
	char screennum[NAME_LEN];
	int allowrepeat; //1只可以运行一次本程序，0，多次
	int printmode; //0为驱动打印；1串口打印；2直接打印；3文件打印；4服务打印；
	char fprintname[NAME_LEN];
	char fpshare[NAME_LEN];
	int printcom;
	int autologin;
	char loginprompt[128];
	char passwordprompt[128];
	char autousername[NAME_LEN];
	char autopassword[NAME_LEN];
	...
	char comname01y[NAME_LEN];
	char comname02z[NAME_LEN];
	char comname03x[NAME_LEN];
	char comname04u[NAME_LEN];
	...
	char keycodedelete[KEY_LEN];
	char keycodehome[KEY_LEN];
	char keycodespace[KEY_LEN];
	char keycodenumlock[KEY_LEN];
	char keycodeprintscreen[KEY_LEN];
	char keycodescroll[KEY_LEN];
	char keycodeend[KEY_LEN];
	char keycodepageup[KEY_LEN];
	char keycodepagedown[KEY_LEN];
	char keycodediv[KEY_LEN];
	char keycodemult[KEY_LEN];
	char keycodeadd[KEY_LEN];
	char keycodesub[KEY_LEN];
	char keycodepause[KEY_LEN];
	char keycodeup[KEY_LEN];
	char keycodedown[KEY_LEN];
	char keycodeleft[KEY_LEN];
	char keycoderight[KEY_LEN];
	/* Basic options */
	char host[512];
	int port;
	int protocol;
	...
	int wrap_mode;
	int lfhascr;
	int cursor_on; /* 0=undisplay,1,display*/
	...
	int sshbug_ignore1, sshbug_plainpw1, sshbug_rsa1, sshbug_hmac2,
			sshbug_derivekey2, sshbug_rsapad2, sshbug_pksessid2, sshbug_rekey2;
...
GLOBAL char configpath[MAX_PATH];
GLOBAL char currentpath[MAX_PATH];

GLOBAL char sysusername[NAME_LEN];
/*
 * This is set TRUE by cmdline.c iff a session is loaded with "-load".
 */GLOBAL int loaded_session;

GLOBAL int enablekey;
struct RSAKey;
...
void term_mouse(Terminal *, Mouse_Button, Mouse_Button, Mouse_Action, int, int,
		int, int, int);
...
enum {
	PKT_INCOMING, PKT_OUTGOING
};
enum {
	PKTLOG_EMIT, PKTLOG_BLANK, PKTLOG_OMIT
};
struct logblank_t {
	int offset;
	int len;
...
void log_packet(void *logctx, int direction, int type, char *texttype,
		void *data, int len, int n_blanks, const struct logblank_t *blanks);
 ...
 */
void ser_setup_config_box(struct controlbox *b, int midsession, int parity_mask,
		int flow_mask);
...
int mb_to_wc(int codepage, int flags, char *mbstr, int mblen, wchar_t *wcstr,
		int wclen);
int wc_to_mb(int codepage, int flags, wchar_t *wcstr, int wclen, char *mbstr,
 ...
 */
struct controlbox;
void setup_config_box(struct controlbox *b, int midsession, int protocol,
		int protcfginfo);
 ...
 */
enum {
	X11_NO_AUTH, X11_MIT, /* MIT-MAGIC-COOKIE-1 */
