...
struct LogContext {
	FILE *lgfp;
	enum {
		L_CLOSED, L_OPENING, L_OPEN, L_ERROR
	} state;
	bufchain queue;
...
static void logfopen_callback(void *handle, int mode) {
	...
	event = dupprintf("%s session log (%s mode) to file: %s",
			...
			(ctx->cfg.logtype == LGTYP_ASCII ? "ASCII" :
				...
				ctx->cfg.logtype == LGTYP_SSHRAW ? "SSH raw data" : "unknown"),
...
void log_eventlog(void *handle, const char *event) {
	...
	if (!ctx)
		return;
	if (ctx->cfg.logtype != LGTYP_PACKETS && ctx->cfg.logtype != LGTYP_SSHRAW)
		return;
 ...
 */
void log_packet(void *handle, int direction, int type, char *texttype,
	...
	if (texttype)
		logprintf(ctx, "%s packet type %d / 0x%02x (%s)\r\n",
				direction == PKT_INCOMING ? "Incoming" : "Outgoing", type, type,
				texttype);
	else
	...
	while (p < len) {
		...
		while ((b < n_blanks) && (p >= blanks[b].offset + blanks[b].len))
			b++;
		/* Work out what type of blanking to apply to
		 * this byte. */
		blktype = PKTLOG_EMIT; /* default */
		if ((b < n_blanks) && (p >= blanks[b].offset)
 ...
 */
static void xlatlognam(Filename *dest, Filename src, char *hostname,
		struct tm *tm) {
	char buf[10], *bufp;
	int size;
