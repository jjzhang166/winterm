...
static const char *serial_configure(Serial serial, HANDLE serport, Config *cfg) {
	...
	if (GetCommState(serport, &dcb)) {
		...
		case 4:
			...
			break;
		default:
			return "Invalid number of stop bits (need 1, 1.5 or 2)";
		}
...
static const char *serial_init(void *frontend_handle, void **backend_handle,
	...
	*backend_handle = serial;
...

	{
		 ...
		 */
		char *serfilename = dupprintf("%s%s",
				strchr(cfg->serline, '\\') ? "" : "\\\\.\\", cfg->serline);
...
static void serial_special(void *handle, Telnet_Special code) {
	...
	if (serial->port && code == TS_BRK) {
		 ...
		 * and so will I. 
		 */
		serial->clearbreak_time = schedule_timer(TICKSPERSEC * 2 / 5,
				serbreak_timer, serial);
...
static const struct telnet_special *serial_get_specials(void *handle) {
	static const struct telnet_special specials[] = { { "Break", TS_BRK }, {
			NULL, TS_EXITMENU } };
	return specials;
}
...
Backend serial_backend = { serial_init, serial_free, serial_reconfig,
		serial_send, serial_sendbuffer, serial_size, serial_special,
		serial_get_specials, serial_connected, serial_exitcode, serial_sendok,
		serial_ldisc, serial_provide_ldisc, serial_provide_logctx,
		serial_unthrottle, serial_cfg_info, 1 };
