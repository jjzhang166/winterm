...
typedef HRESULT (WINAPI *p_SHGetFolderPath_t)(HWND, int, HANDLE, DWORD, LPTSTR);
...
void write_setting_i(void *handle, const char *key, int value) {
	if (handle)
		RegSetValueEx((HKEY) handle, key, 0, REG_DWORD, (CONST BYTE *) &value,
				sizeof(value));
}
...
char *read_setting_s(void *handle, const char *key, char *buffer, int buflen) {
	...
	if (!handle
			|| RegQueryValueEx((HKEY) handle, key, 0, &type, buffer,
					&size) != ERROR_SUCCESS ||
					type != REG_SZ)
		return NULL;
	else
...
int read_setting_i(void *handle, const char *key, int defvalue) {
	...
	if (!handle
			|| RegQueryValueEx((HKEY) handle, key, 0, &type, (BYTE *) &val,
					&size) != ERROR_SUCCESS
...
static void hostkey_regname(char *buffer, const char *hostname, int port,
		const char *keytype) {
	int len;
...
int verify_host_key(const char *hostname, int port, const char *keytype,
		const char *key) {
...
void store_host_key(const char *hostname, int port, const char *keytype,
		const char *key) {
	char *regname;
	HKEY rkey;
...
static int try_random_seed(char const *path, int action, HANDLE *ret) {
	...
	*ret = CreateFile(path, action == OPEN_W ? GENERIC_WRITE : GENERIC_READ,
	action == OPEN_W ? 0 : (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL,
			action == OPEN_W ? CREATE_ALWAYS : OPEN_EXISTING,
			action == OPEN_W ? FILE_ATTRIBUTE_NORMAL : 0, NULL);
...
static HANDLE access_random_seed(int action) {
	...
	if (RegOpenKey(HKEY_CURRENT_USER, PUTTY_REG_POS, &rkey) == ERROR_SUCCESS) {
		int ret = RegQueryValueEx(rkey, "RandSeedFile", 0, &type, seedpath,
				(LPBYTE) &size);
		if (ret != ERROR_SUCCESS || type != REG_SZ)
			seedpath[0] = '\0';
		RegCloseKey(rkey);
	...
	if (!tried_shgetfolderpath) {
		...
		if (shell32_module) {
			p_SHGetFolderPath = (p_SHGetFolderPath_t) GetProcAddress(
					shell32_module, "SHGetFolderPathA");
