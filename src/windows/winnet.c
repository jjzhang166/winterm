...
DECL_WINSOCK_FUNCTION(static, int, getnameinfo,
		(const struct sockaddr FAR * sa, socklen_t salen, char FAR * host, size_t hostlen, char FAR * serv, size_t servlen, int flags));
...
char *winsock_error_string(int error) {
	...
	case WSAEHOSTUNREACH:
		return "Network error: 请检查本机是否与服务器连通！";
	case WSAEINPROGRESS:
	...
	*realhost = '\0';
	if ((a = p_inet_addr(host)) == (unsigned long) INADDR_NONE) {
		...
		if (ret_family == AF_UNSPEC) {
			ret->error = (
					err == WSAENETDOWN ? "Network is down" :
...
void sk_getaddr(SockAddr addr, char *buf, int buflen) {
	...
	if (addr->ai) {
		if (p_WSAAddressToStringA) {
			p_WSAAddressToStringA(addr->ai->ai_addr, addr->ai->ai_addrlen, NULL,
					buf, &buflen);
		} else
	...
	if (addr->family == AF_INET) {
		struct in_addr a;
		if (!(addr->addresses && addr->curraddr < addr->naddresses)) {
			return;
		}
	...
	if (!n_local_interfaces) {
		...
		if (p_WSAIoctl
				&& p_WSAIoctl(s, SIO_GET_INTERFACE_LIST, NULL, 0,
						local_interfaces, sizeof(local_interfaces), &retbytes,
						NULL, NULL) == 0)
...
Socket sk_register(void *sock, Plug plug) {
	static const struct socket_function_table fn_table = { sk_tcp_plug,
			sk_tcp_close, sk_tcp_write, sk_tcp_write_oob, sk_tcp_flush,
			sk_tcp_set_private_ptr, sk_tcp_get_private_ptr, sk_tcp_set_frozen,
			sk_tcp_socket_error };
...
static DWORD try_connect(Actual_Socket sock) {
...
#ifndef NO_IPV6
		sockcode = p_bind(s,
				(sock->addr->family == AF_INET6 ?
						(struct sockaddr *) &a6 : (struct sockaddr *) &a),
				(sock->addr->family == AF_INET6 ? sizeof(a6) : sizeof(a)));
#else
	...
	if (sock->addr->ai) {
		if (family == AF_INET6) {
			...
			a6.sin6_flowinfo =
					((struct sockaddr_in6 *) sock->addr->ai->ai_addr)->sin6_flowinfo;
			a6.sin6_scope_id =
					((struct sockaddr_in6 *) sock->addr->ai->ai_addr)->sin6_scope_id;
		} else {
			a.sin_family = AF_INET;
			a.sin_addr =
...
#endif
	{
		if (!(sock->addr->addresses
				&& sock->addr->curraddr < sock->addr->naddresses)) {
			return err;
		}
		//assert(sock->addr->addresses && sock->addr->curraddr < sock->addr->naddresses);
		a.sin_family = AF_INET;
		a.sin_addr.s_addr = p_htonl(
				sock->addr->addresses[sock->addr->curraddr]);
		a.sin_port = p_htons((short) sock->port);
	}
...
Socket sk_new(SockAddr addr, int port, int privport, int oobinline, int nodelay,
		int keepalive, Plug plug) {
	static const struct socket_function_table fn_table = { sk_tcp_plug,
			sk_tcp_close, sk_tcp_write, sk_tcp_write_oob, sk_tcp_flush,
			sk_tcp_set_private_ptr, sk_tcp_get_private_ptr, sk_tcp_set_frozen,
			sk_tcp_socket_error };
	...
	InitializeCriticalSection(&ret->critsec);
...
static void sk_tcp_close(Socket sock) {
	...
	DeleteCriticalSection(&s->critsec);
	if (s->child)
		...
		if (nsent <= 0) {
			...
			if ((err < WSABASEERR && nsent < 0) || err == WSAEWOULDBLOCK) {
				...
				return;
					...
					== 0|| err == WSAECONNABORTED || err == WSAECONNRESET) {
			/*
...
static int sk_tcp_write(Socket sock, const char *buf, int len) {
	...
	int size = bufchain_size(&s->output_data);
	LeaveCriticalSection(&s->critsec);
...
int select_result(WPARAM wParam, LPARAM lParam) {
	...
	if ((err = WSAGETSELECTERROR(lParam)) != 0) {
		 ...
		 */
		if (s->addr) {
			plug_log(s->plug, 1, s->addr, s->port, winsock_error_string(err),
					err);
