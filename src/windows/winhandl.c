...
static DWORD WINAPI handle_output_threadfunc(void *param) {
	...
	while (1) {
		...
		if (povl)
			memset(povl, 0, sizeof(OVERLAPPED));
		writeret = WriteFile(ctx->h, ctx->buffer, ctx->len, &ctx->lenwritten,
				povl);
		if (!writeret)
		...
		if (povl && !writeret && GetLastError() == ERROR_IO_PENDING) {
			writeret = GetOverlappedResult(ctx->h, povl, &ctx->lenwritten,
					TRUE);
			if (!writeret)
				ctx->writeerr = GetLastError();
			else
...
struct handle *handle_output_new(HANDLE handle, handle_outputfn_t sentdata,
	...
	CreateThread(NULL, 0, handle_output_threadfunc, &h->u.i, 0, &out_threadid);
