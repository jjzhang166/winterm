...
void ldisc_send_bin(void *handle, unsigned char *buf, int len, int interactive);
...
static void pwrite(Ldisc ldisc, unsigned char c) {
	if ((c >= 32 && c <= 126) || (!in_utf(ldisc->term) && c >= 0xA0)
...
void *ldisc_create(Config *mycfg, Terminal *term, Backend *back,
		void *backhandle, void *frontend) {
	...
	if (EDITING) {
		while (len--) {
			...
			case KCTRL('?'): /* backspace/delete */
				if (ldisc->buflen > 0) {
					do {
						if (ECHOING)
							bsb(ldisc,
									plen(ldisc, ldisc->buf[ldisc->buflen - 1]));
						ldisc->buflen--;
			...
			case CTRL('W'): /* delete word */
				while (ldisc->buflen > 0) {
					...
					if (ldisc->buflen > 0
							&& isspace(
									(unsigned char) ldisc->buf[ldisc->buflen - 1])
							&& !isspace(
			...
			case CTRL('D'): /* logout or send */
				if (ldisc->buflen == 0) {
					...
					ldisc->back->send(ldisc->backhandle, ldisc->buf,
							ldisc->buflen);
					ldisc->buflen = 0;
				}
				break;
			...
			default: /* get to this label from ^V handler */
				default_case: if (ldisc->buflen >= ldisc->bufsiz) {
		...
		if (len > 0) {
			...
			if (keyflag && ldisc->cfg->protocol == PROT_TELNET && len == 1) {
				switch (buf[0]) {
				case CTRL('M'):
					if (ldisc->cfg->protocol == PROT_TELNET
							&& ldisc->cfg->telnet_newline)
						ldisc->back->special(ldisc->backhandle, TS_EOL);
					else
						ldisc->back->send(ldisc->backhandle, "\r", 1);
	...
	if (EDITING) {
		...
		if (len > 0) {
			...
			if (keyflag && ldisc->cfg->protocol == PROT_TELNET && len == 1) {
				...
				ldisc->back->send_bin(ldisc->backhandle, buf, len);
		}
	}
}
