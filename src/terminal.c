...
static void term_com_setup(Terminal *term);
static void term_com_flush(Terminal *term);
static void term_com_finish(Terminal *);
	static void term_print_finish(Terminal *); //xh
...
static void makerle(struct buf *b, termline *ldata,
		void (*makeliteral)(struct buf *b, termchar *c, unsigned long *state)) {
		...
		if (thislen == prevlen
			...
			if (thislen > 1 || prev2) {
				...
				while (n > 0 && runlen < 129) {
					...
					b->len = tmppos;
					if (tmplen != thislen
							|| memcmp(b->data + runpos + 1, b->data + tmppos,
									tmplen)) {
						state = oldstate;
						break; /* run over */
					}
...
static unsigned char *compressline(termline *ldata) {
...
#ifndef CHECK_SB_COMPRESSION
	{
		...
		//assert(b->len == dused);
		if(ldata->cols != dcl->cols)
		{
			return 0;
		}
		//assert(ldata->cols == dcl->cols);
		if(ldata->lattr != dcl->lattr)
		{
			return 0;
		}
		...
		{
			if(!(termchars_equal(&ldata->chars[i], &dcl->chars[i])))
			{
				return;
			}
...
static termline *lineptr(Terminal *term, int y, int lineno, int screen) {
	...
	if (y >= 0) {
		...
		if (term->cfg.erase_to_scrollback && term->alt_which
				&& term->alt_screen) {
	...
	if (line == NULL) {
		fatalbox("line==NULL in terminal.c\n"
				...
				"and pass on the above information.", lineno, y, term->cols,
				term->rows, term->scrollback, count234(term->scrollback),
				term->screen, count234(term->screen), term->alt_screen,
				count234(term->alt_screen), term->alt_sblines, whichtree,
				treeindex);
	}
	//////////xh/////////////////
	if (line == NULL) {
...
static void power_on(Terminal *term, int clear) {
	...
	term->rvideo = 0;
	term->in_vbell = FALSE;
	term->cursor_on = cfg.cursor_on;
	term->big_cursor = 0;
	term->default_attr = term->save_attr = term->alt_save_attr =
...
Terminal *term_init(Config *mycfg, struct unicode_data *ucsdata, void *frontend) {
	...
	term->printer_pbuf = smalloc(sizeof(bufchain));
	bufchain_init(term->printer_pbuf);
	bufchain_init(&term->com_buf);
	term->printing = term->only_printing = FALSE;
	term->only_coming = TRUE;
	...
	term->basic_erase_char.zh = 0;
	term->erase_char = term->basic_erase_char;
	term->cfg.beep = cfg.beep;

	term->autologin = cfg.autologin;
	term->login = FALSE;
	term->cfg.blinktext = cfg.blinktext;

	term->cfg.allowrepeat = cfg.allowrepeat;
...
//	bufchain_clear(term->printer_pbuf);
//	sfree(term->printer_pbuf);
...
void term_size(Terminal *term, int newrows, int newcols, int newsavelines) {
	...
	//防止报出异常xh////////////
	if (!(term->rows == count234(term->screen))) {
		return;
	}
	///////////////////
//	assert(term->rows == count234(term->screen));//xh
	...
	if (!(count234(term->scrollback) <= newsavelines)) {
		return;
	}
	// assert(count234(term->scrollback) <= newsavelines);
	if (!(count234(term->scrollback) >= term->tempsblines)) {
		return;
	}
 ...
 */
void term_provide_resize_fn(Terminal *term, void (*resize_fn)(void *, int, int),
		void *resize_ctx) {
...
static void update_sbar(Terminal *term) {
	...
	set_sbar(term->frontend, nscroll + term->rows, nscroll + term->disptop,
			term->rows);
}
 ...
 */
static void erase_lots(Terminal *term, int line_only, int from_begin,
		int to_end) {
	pos start, end;
	int erase_lattr;
...
static void toggle_mode(Terminal *term, int mode, int query, int state) {
	if (query)
		...
		case 47: /* alternate screen */
			compatibility(OTHER);
			deselect(term);
			swap_screen(term, term->cfg.no_alt_screen ? 0 : state, FALSE,
					FALSE);
			term->disptop = 0;
			break;
		case 50:
...
static void term_com_finish(Terminal *term) {
	void *data;
	int len, size;
	char c;
	...
	term_com_flush(term);
	while ((size = bufchain_size(&term->com_buf)) > 0) {
		bufchain_prefix(&term->com_buf, &data, &len);
		c = *(char *) data;
		if (c == '\033') {
 ...
 * ANSI printing routines.
 */
void term_print_setup(Terminal *term) {
	bufchain_clear(term->printer_pbuf);
	term->printing = TRUE;
	term->print_state = 0;
	print_open(term->printer);
}
...
static void term_print_flush(Terminal *term) {
	...
	int saved = term->print_state + term->print_saved;
	while ((size = bufchain_size(term->printer_pbuf)) > saved) {
		bufchain_prefix(term->printer_pbuf, &data, &len);
		if (len > size - saved)
			len = size - saved;
		print_data(term->printer, data, len);
		bufchain_consume(term->printer_pbuf, len);
	}
}
...
static void term_print_finish(Terminal *term) {
	...
	while ((size = bufchain_size(term->printer_pbuf)) > 0) {
		bufchain_prefix(term->printer_pbuf, &data, &len);
		c = *(char *) data;
		if (c == '\033' || c == '\233') {
			bufchain_consume(term->printer_pbuf, size);
			break;
		} else {
			print_data(term->printer, &c, 1);
			bufchain_consume(term->printer_pbuf, 1);
		}
	}
...
static void term_out(Terminal *term) {
	...
	while (nchars > 0 || unget != -1 || bufchain_size(&term->inbuf) > 0) {
		 ...
		 */
		if (term->printing) {
			bufchain_add(term->printer_pbuf, &c, 1);
		...
		if (comapi_writing_enable()) {
			...
			if (term->only_coming) {
				if (c == '\033')
					term->com_state = 1;
				else if (c == '[' && term->com_state == 1)
					term->com_state = 2;
				else if (c == '/' && term->com_state == 2)
					term->com_state = 3;
				else if (c == '5' && term->com_state == 3)
					term->com_state = 4;
				else if (c == '1' && term->com_state == 4)
					term->com_state = 5;
				else if (c == 'l' && term->com_state == 5) {
					term->com_state = 6;
				} else {
					term->com_state = 0;
				}
		...
		!term->vt52_mode && has_compat(VT220)) {if (c == 0x9a)
		c = 0;
		else {
		...
		if (c == '\177' && term->termstate < DO_CTRLS && has_compat(OTHER)) {
			...
			if (!term->cfg.no_dbackspace) {
				...
				copy_termchar(scrlineptr(term->curs.y), term->curs.x,
						&term->erase_char);
			}
		} else
		...
		if ((c & ~0x1F) == 0 && term->termstate < DO_CTRLS) {
			...
			case '\005': /* ENQ: terminal type query */
				...
				if (term->ldisc) {
					...
					lpage_send(term->ldisc, DEFAULT_CODEPAGE, abuf, d - abuf,
							0);
				}
				break;
			case '\007': /* BEL: Bell */
				 ...
				 * ctrls are stripped above */
			{
				...
				case 1:
					...
					if (term->curs.x > 0
							&& cline->chars[term->curs.x - 1].zh == 1) {
						cline->chars[term->curs.x].zh = 2;
						int full = cline->chars[term->curs.x - 1].chr;
						full = (c & 0xFF) | ((full & 0xFF) << 8);
						cline->chars[term->curs.x].full = full;
						cline->chars[term->curs.x - 1].full = full;
					} else {
						BYTE byte = (BYTE) (cline->chars[term->curs.x].chr
								& 0xFF);
						if (IsDBCSLeadByteEx(936, byte)) {
							cline->chars[term->curs.x].zh = 1;
						} else {
							cline->chars[term->curs.x].zh = 0;
						}
					}
			...
			case SEEN_ESC:
				...
				case '>': /* DECKPNM: Keypad numeric mode */
					...
					break;
				case 'D': /* IND: exactly equivalent to LF */ //左键
					compatibility(VT100);
					if (term->curs.y == term->marg_b)
						scroll(term, term->marg_t, term->marg_b, 1, TRUE);
					else if (term->curs.y < term->rows - 1)
				...
				if (isdigit(c)) {
					if (term->esc_nargs <= ARGS_MAX) {
						...
						term->esc_args[term->esc_nargs - 1] = 10
								* term->esc_args[term->esc_nargs - 1] + c - '0';
					}
					term->termstate = SEEN_CSI;
				} else if (c == ' ') {
					term->termstate = SEEN_CSI;
					...
					case 'C': /* CUF: Cursor right */
						//erase_lots(term, TRUE, 0, term->curs.x + def(term->esc_args[0], 1));
					...
					case 'E': /* CNL: move down N lines and CR */
						compatibility(ANSI);
						move(term, 0, term->curs.y + def(term->esc_args[0], 1),
								1);
						seen_disp_event(term);
						break;
					...
					case '`': /* HPA: set horizontal posn */
						compatibility(ANSI);
						move(term, def(term->esc_args[0], 1) - 1, term->curs.y,
								0);
						seen_disp_event(term);
						break;
					...
					case 'n': /* DSR: cursor position query */
						...
						break;
					case ANSI('h', '/'): //zhangbo
						toggle_mode(term, term->esc_args[term->esc_nargs - 1],
								term->esc_query, TRUE);
						break;
					case ANSI('l', '/'): //zhangbo
						toggle_mode(term, term->esc_args[term->esc_nargs - 1],
					...
					case 'm': /* SGR: set graphics rendition */
					{
						...
						for (i = 0; i < term->esc_nargs; i++) {
							...
							case 37:
								...
								term->curr_attr |= (term->esc_args[i] - 30)
										<< ATTR_FGSHIFT;
								break;
							case 90:
							case 91:
							...
							case 47:
								...
								term->curr_attr |= (term->esc_args[i] - 40)
										<< ATTR_BGSHIFT;
								break;
							case 100:
							case 101:
							...
							case 38: /* xterm 256-colour mode */
								if (i + 2 < term->esc_nargs
										&& term->esc_args[i + 1] == 5) {
									term->curr_attr &= ~ATTR_FGMASK;
									term->curr_attr |= ((term->esc_args[i + 2]
											& 0xFF) << ATTR_FGSHIFT);
									i += 2;
								}
								break;
					...
					case 't': /* DECSLPP: set page size - ie window height */
						...
						case 18:
							if (term->ldisc) {
								len = sprintf(buf, "\033[8;%d;%dt", term->rows,
										term->cols);
								ldisc_send(term->ldisc, buf, len, 0);
							}
							break;
						...
						case 20:
							if (term->ldisc
									&& term->cfg.remote_qtitle_action
											!= TITLE_NONE) {
								if (term->cfg.remote_qtitle_action
										== TITLE_REAL)
					...
					case 'X': /* ECH: write N spaces w/o moving cursor */
						...
						compatibility(ANSIMIN);
						{
							...
							cursplus.x += n;
							check_boundary(term, term->curs.x, term->curs.y);
							check_boundary(term, term->curs.x + n,
									term->curs.y);
							check_selection(term, term->curs, cursplus);
							while (n--)
					...
					case ANSI('F', '='): /* set normal foreground */
						...
						if (term->esc_args[0] >= 0 && term->esc_args[0] < 16) {
							long colour = (sco2ansicolour[term->esc_args[0]
									& 0x7] | (term->esc_args[0] & 0x8))
									<< ATTR_FGSHIFT;
							term->curr_attr &= ~ATTR_FGMASK;
 ...
 */
static int term_bidi_cache_hit(Terminal *term, int line, termchar *lbefore,
		int width) {
	int i;
...
static void term_bidi_cache_store(Terminal *term, int line, termchar *lbefore,
		termchar *lafter, bidi_char *wcTo, int width, int size) {
	int i;
	...
	if (!term->pre_bidi_cache || term->bidi_cache_size <= line) {
		...
		while (j < term->bidi_cache_size) {
			term->pre_bidi_cache[j].chars = term->post_bidi_cache[j].chars =
					NULL;
			term->pre_bidi_cache[j].width = term->post_bidi_cache[j].width = -1;
			term->pre_bidi_cache[j].forward = term->post_bidi_cache[j].forward =
					NULL;
...
static termchar *term_bidi_line(Terminal *term, struct termline *ldata,
	...
	if (!term->cfg.bidi || !term->cfg.arabicshaping) {
		...
		if (!term_bidi_cache_hit(term, scr_y, ldata->chars, term->cols)) {
			...
			term_bidi_cache_store(term, scr_y, ldata->chars, term->ltemp,
					term->wcTo, term->cols, ldata->size);
...
//		if (lchars[our_curs_x].zh == 1) {
//			lchars[our_curs_x].attr |= ATTR_WIDE;
//		}
	...
	for (i = 0; i < term->rows; i++) {
		...
		if (term->selstate == DRAGGING || term->selstate == SELECTED) {
			if (scrpos.y == term->selstart.y) {
				termchar *d = lchars + term->selstart.x;
				if (d->zh == 2) {
					term->selstart.x++;
				}
			}
			if (scrpos.y == term->selend.y) {
				termchar *d = lchars + term->selend.x - 1;
				if (d->zh == 1) {
					term->selend.x++;
				}
			}
		...
		for (j = 0; j < term->cols; j++) {
			...
			if (!term->cfg.ansi_colour)
				tattr = (tattr & ~(ATTR_FGMASK | ATTR_BGMASK)) | ATTR_DEFFG
						| ATTR_DEFBG;
		...
		for (j = 0; j < term->cols; j++) {
			...
			//当前字符与显示字符在宽度上不一致，dirty_line = true
			if ((term->disptext[i]->chars[j].attr ^ tattr) & ATTR_WIDE)
				dirty_line = TRUE;

			//当前字符与开始字符在属性上不一致，break_run = true
			...
			do_copy = FALSE;
			if (!termchars_equal_override(&term->disptext[i]->chars[j], d,
					tchar, tattr)) {
				do_copy = TRUE;
				dirty_run = TRUE;
			}
			...
			if (ccount == 0 && zh == 2) {
				ch[ccount++] = term->ucsdata->unitab_line[(unsigned char) ' '];
			} else {
			...
			if (d->cc_next) {
				...
				while (dd->cc_next) {
					...
					if (ccount == 0 && zh == 2) {
						ch[ccount++] =
								term->ucsdata->unitab_line[(unsigned char) ' '];
					} else {
 ...
 */
void term_paint(Terminal *term, Context ctx, int left, int top, int right,
		int bottom, int immediately) {
	int i, j;
	if (left < 0)
		left = 0;
	if (top < 0)
	...
	for (i = top; i <= bottom && i < term->rows; i++) {
		if ((term->disptext[i]->lattr & LATTR_MODE) == LATTR_NORM)
			...
			for (j = left; j <= right + 1 && j < term->cols; j++)
				term->disptext[i]->chars[j].attr |= ATTR_INVALID;
	}
			...
			while (1) {
				...
				if (DIRECT_FONT(uc)) {
					if (c >= ' ' && c != 0x7F) {
						...
						WCHAR wbuf[4];
						int rv;
						if (is_dbcs_leadbyte(term->ucsdata->font_codepage,
								(BYTE) c)) {
							buf[0] = c;
							buf[1] =
									(char) (0xFF & ldata->chars[top.x + 1].chr);
							rv = mb_to_wc(term->ucsdata->font_codepage, 0, buf,
									2, wbuf, 4);
							top.x++;
						} else {
							buf[0] = c;
				...
				for (p = cbuf; *p; p++)
					clip_addchar(&buf, *p, attr); //把选中字符放入剪贴板
...
static void printme(Terminal *term, pos top, pos bottom, int rect, int desel) {
	clip_workbuf buf;
	int old_top_x;
	int attr;
	int buflen = 0;
...
//////////////////////////////////////////////////
	...
	buf.bufpos = 0;
	buf.textptr = buf.textbuf = snewn(buf.buflen, wchar_t);
	memset(buf.textbuf, 0x00, buf.buflen);
	buf.attrptr = buf.attrbuf = snewn(buf.buflen, int);
	...
	clip_addchar(&buf, 0, 0);
#endif
	buflen = WideCharToMultiByte(CP_ACP, 0, buf.textbuf, buf.bufpos, 0, 0, NULL,
			NULL);
	buffer = smalloc(buflen);
	WideCharToMultiByte(CP_ACP, 0, buf.textbuf, buf.bufpos, buffer, buflen,
			NULL, NULL);

	print_open(term->printer);
	print_data(term->printer, prefix, prelen);
	print_data(term->printer, buffer, buflen);
	print_data(term->printer, suffix, suflen);
	print_close(term->printer);
...
void term_copy_screen(Terminal *term) {
	pos top;
	pos bottom;
...
void term_print_screen(Terminal *term) {
	pos top;
	pos bottom;
	tree234 *screen = term->screen;
	top.y = 0; //-sblines(term);
	top.x = 0;
	bottom.y = find_last_nonempty_line(term, screen);
	bottom.x = term->cols;
	printme(term, top, bottom, 0, TRUE);
}
/*
...
static int wordtype(Terminal *term, int uc) {
	...
	static const struct ucsword ucs_words[] = { { 128, 160, 0 },
			{ 161, 191, 1 }, { 215, 215, 1 }, { 247, 247, 1 }, { 0x037e, 0x037e,
					1 }, /* Greek question mark */
...
void term_select_copy(Terminal *term, int x, int y) {
	if (term->selstate == DRAGGING) {
		if (y < term->selanchor.y
				|| (y == term->selanchor.y && x < term->selanchor.x)) {
			term->selstart.x = x;
			term->selstart.y = y;
...
void term_do_copy(Terminal *term) {
	if (term->selstate == SELECTED && (!poseq(term->selstart,term->selend))) {
...
//		deselect(term);
		...
		if (term->selstate == DRAGGING) {
			...
			term->selstate = DRAGGING;
			term->selstart = term->selend = selpoint;
...
//		sel_spread(term);
		...
		if (term->seltype == LEXICOGRAPHIC) {
			...
			term->selend.x = 2 + max(term->selanchor.x, selpoint.x);
		...
		if (term->selstate == DRAGGING) {
			term->selstate = SELECTED;
			if ((cfg.autocopy == 1) && (!poseq(term->selstart,term->selend))) {
				clipme(term, term->selstart, term->selend,
						(term->seltype == RECTANGULAR), FALSE);
			}
 ...
 * input.
 */
int term_get_userpass_input(Terminal *term, prompts_t *p, unsigned char *in,
		int inlen) {
...
void autologinpass(char *outbuf, int outbuflen) {
	unsigned int reg = 0;
	if (term->autologin == 1 && (strlen(cfg.loginprompt) != 0)
			&& (strlen(cfg.passwordprompt) != 0)) {
		reg = memcmp(outbuf + outbuflen - strlen(cfg.loginprompt),
				cfg.loginprompt, strlen(cfg.loginprompt));
		if (reg == 0) {
			back->send(backhandle, cfg.autousername, strlen(cfg.autousername));
			if (cfg.autousername[strlen(cfg.autousername) - 1] != '\n')
				back->send(backhandle, "\n", 1);
		}
		reg = memcmp(outbuf + outbuflen - strlen(cfg.passwordprompt),
				cfg.passwordprompt, strlen(cfg.passwordprompt));
		if (reg == 0
				|| memcmp(outbuf + 2, cfg.passwordprompt,
						strlen(cfg.passwordprompt)) == 0) {
			back->send(backhandle, cfg.autopassword, strlen(cfg.autopassword));
			if (cfg.autopassword[strlen(cfg.autopassword) - 1] != '\n')
				back->send(backhandle, "\n", 1);
		}
	}
}
