...
static int rlogin_receive(Plug plug, int urgent, byte *data, int len) {
	Rlogin rlogin = (Rlogin) plug;
	if (urgent == 2) {
		char c;
...
static const char *rlogin_init(void *frontend_handle, void **backend_handle,
		...
		int keepalive) {
	static const struct plug_function_table fn_table = { rlogin_log,
	...
	*backend_handle = rlogin;
	 ...
	 * Open socket.
	 */
	rlogin->s = new_connection(addr, *realhost, port, 1, 0, nodelay, keepalive,
			(Plug) rlogin, cfg);
...
Backend rlogin_backend = { rlogin_init, rlogin_free, rlogin_reconfig,
		rlogin_send, rlogin_sendbuffer, rlogin_size, rlogin_special,
		rlogin_get_specials, rlogin_connected, rlogin_exitcode, rlogin_sendok,
		rlogin_ldisc, rlogin_provide_ldisc, rlogin_provide_logctx,
		rlogin_unthrottle, rlogin_cfg_info, 1 };
